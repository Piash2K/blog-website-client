...
const AllBlogs = () => {
  ...
  const [sortOption, setSortOption] = useState(""); // New: Added sortOption state
  ...
  // Fetch blogs based on pagination, category, search query, and sort option
  useEffect(() => {
    setIsLoading(true);
    const query = `https://blog-website-server-nine.vercel.app/blogs?page=${currentPage}&size=${itemsPerPage}${selectedCategory ? `&category=${encodeURIComponent(selectedCategory)}` : ""
      }${searchQuery ? `&search=${searchQuery}` : ""}${sortOption ? `&sort=${sortOption}` : ""}`;
  ...
  }, [currentPage, itemsPerPage, selectedCategory, searchQuery, sortOption]); // Added sortOption to dependencies
